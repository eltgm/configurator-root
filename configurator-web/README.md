...
Frontend Configurator на React, TypeScript и Vite. На этапе 9.13 доступны application shell,
первый запуск, управление предметными областями, типами компонентов и их атрибутами.
...
тёмную темы, а также русский и английский языки. Выор сохраняется в `localStorage`. Текущая предметная область
выбирается в header и определяет данные страницы `/settings/types`.
