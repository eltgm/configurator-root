...
Frontend Configurator на React, TypeScript и Vite. Реализованы предметные области, каталог компонентов и
переиспользуемых атрибутов, совместимость, assembly-aware конфигуратор, конфигурации, адаптивный AppShell и production
NGINX gateway.
...
После выбора первого компонента `/configurator` получает assembly-aware кандидатов от backend. Основной список
содержит только доступные варианты, неприменимые отношения скрываются, а заблокированные кандидаты показываются
отдельно с точным объяснением. Сохранение допускает связный граф разрешённых отношений; прямая связь между каждой
парой компонентов не требуется, но любое применимое несовпадение автоматического правила блокирует сборку.
