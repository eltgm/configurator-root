...
class ConfigurationControllerExternalIntegrationSpec extends AbstractConfigurationControllerContract {
    ...
    TestResponse get(String path) {
        ...
        def headers = response.headers.asList().collectEntries { header -> [(header.name): header.value] }
        return new TestResponse(response.statusCode(), response.asString(), headers)
    }
