...
import ru.sultanyarov.configurator.contract.AbstractConfigurationControllerContract
...
class ConfigurationControllerIntegrationSpec extends AbstractConfigurationControllerContract {
    @Autowired
    MockMvc mockMvc
    ...
    TestResponse get(String path) {
        def response = mockMvc.perform(MockMvcRequestBuilders.get(path)).andReturn().response
        def headers = response.headerNames.collectEntries { name -> [(name): response.getHeader(name)] }
        return new TestResponse(response.status, response.contentAsString, headers)
    }
    ...
    TestResponse post(String path, Object body) {
        def response = mockMvc.perform(MockMvcRequestBuilders.post(path)
