...
import ru.sultanyarov.configurator.contract.AbstractCompatibilityControllerContract
...
class CompatibilityControllerIntegrationSpec extends AbstractCompatibilityControllerContract {
    @Autowired
    MockMvc mockMvc
    ...
    TestResponse put(String path, Object body) {
        def result = mockMvc.perform(MockMvcRequestBuilders.put(path)
                .contentType("application/json")
                .content(objectMapper.writeValueAsString(body))).andReturn()
        return new TestResponse(result.response.status, result.response.contentAsString)
    }
