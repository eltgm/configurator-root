...
2. Скачать архив своей ОС, `SHA256SUMS` и при необходимости `IMAGE_DIGESTS` из GitHub Release `v1.1.3`.
...
```powershell
Get-FileHash -Algorithm SHA256 .\configurator-windows-v1.1.3.zip
```

macOS:

```bash
shasum -a 256 configurator-macos-v1.1.3.tar.gz
```
...
gh attestation verify oci://ghcr.io/eltgm/configurator-web:1.1.3 -R eltgm/configurator-root
```
...
3. Создать annotated tag на commit из `master`: `scripts/release/start-release-tag.sh 1.1.3`.
