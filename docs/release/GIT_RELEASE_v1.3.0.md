# Git runbook — Configurator v1.3.0
...
```bash
git switch codex/release-1.3.0
git status --short
git diff --check
git push -u origin codex/release-1.3.0
```

Откройте PR `codex/release-1.3.0` → `develop`, дождитесь зелёного CI, review и merge. Затем откройте release PR
...
git status --short
scripts/release/start-release-tag.sh 1.3.0
```

Скрипт требует чистое дерево, точное совпадение с `origin/master`, section 1.3.0 в CHANGELOG, versioned release notes,
OpenAPI и frontend package metadata. Он создаёт annotated tag `v1.3.0` и отправляет только tag.
...
Если проверка не пройдена, исправляйте новый commit и выпускайте новую версию/tag; `v1.3.0` не удаляйте и не двигайте.
