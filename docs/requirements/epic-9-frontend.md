...
> Начиная с `v1.1.0`, исторические требования 9.21–9.25 к полной попарной direct-совместимости заменены
> assembly-aware моделью из `docs/plans/completed/20260825-intuitive-assembly-compatibility.md`: связный граф
> разрешённых отношений, блокировка любого `DENIED` решения и отдельный candidates endpoint.
> Начиная с `v1.3.0`, требования 9.20–9.25 к одному компоненту каждого типа и `componentIds` расширены количеством,
> несколькими моделями одного типа и опциональным учётом остатков. Актуальный контракт описан в
> [`configuration-inventory.md`](configuration-inventory.md).
