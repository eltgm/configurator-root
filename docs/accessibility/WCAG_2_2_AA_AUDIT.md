...
Матрица относится к `configurator-web` и задаче 9.26. Она является внутренним implementation audit, а не внешней
...
- `9.27` — дополнительный автоматический evidence будет добавлен axe/visual regression задачей 9.27.
...
| ------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------- |
| 1.1.1 Non-text Content                | Значимые изображения получают alt; декоративные icons/previews скрыты от AT.                  | Gallery/component tests, manual image audit | manual, 9.27  |
| 1.2.1–1.2.5 Time-based Media          | Аудио и видео отсутствуют.                                                                    | Content inventory                           | n/a           |
| 1.3.1 Info and Relationships          | Landmarks, headings, labels, tables/lists и form associations выражены семантически.          | App/shared/feature tests                    | covered, 9.27 |
...
| 1.4.1 Use of Color                    | State дополняется текстом/icon/role.                                                          | State component tests, manual themes        | manual, 9.27  |
| 1.4.2 Audio Control                   | Автовоспроизводимое аудио отсутствует.                                                        | Content inventory                           | n/a           |
| 1.4.3 Contrast (Minimum)              | Mantine tokens и собственные styles проверяются в light/dark.                                 | Manual contrast sampling                    | manual, 9.27  |
...
| 1.4.11 Non-text Contrast              | Focus, controls и meaningful graphics проверяются в обеих темах.                              | Manual contrast sampling                    | manual, 9.27  |
...
| 2.4.4 Link Purpose (In Context)       | Links имеют предметные labels или доступный context.                                          | Component audit                             | covered, 9.27 |
...
| 2.4.6 Headings and Labels             | Один H1 на странице, labels описывают назначение controls.                                    | App/feature tests                           | covered, 9.27 |
...
| 2.5.3 Label in Name                   | Visible button/link/form labels входят в accessible name.                                     | Component audit                             | covered, 9.27 |
...
| 4.1.1 Parsing                         | React/TypeScript создают валидную DOM structure; duplicate IDs проверяются code review/tests. | Build and component tests                   | covered, 9.27 |
| 4.1.2 Name, Role, Value               | Native/Mantine semantics, ARIA только для дополнения.                                         | Component tests                             | covered, 9.27 |
...
- Task 9.27 adds axe scans, CI accessibility gate and visual regression baselines.
