...
| Web-интерфейс    | В разработке    | shell, темы, области, типы, атрибуты и каталог                                    |
| Аутентификация   | Только контракт | регистрация, login и Bearer JWT описаны, runtime-реализации нет                   |

Предметные страницы совместимости, конфигуратора и конфигураций пока представлены подготовленными
маршрутами. Реальный пользовательский сценарий уже доступен для создания демо/пустой предметной области, выбора,
редактирования и подтверждённого удаления, для настройки типов и атрибутов, а также для поиска, фильтрации,
архивирования и восстановления компонентов каталога. Web-интерфейс также поддерживает создание, полную карточку и
редактирование компонентов с полями, сформированными из атрибутов выбранного типа.
