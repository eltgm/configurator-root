...
| Web-интерфейс    | В разработке    | shell, темы, локализация, предметные области, типы и атрибуты                     |
| Аутентификация   | Только контракт | регистрация, login и Bearer JWT описаны, runtime-реализации нет                   |

Предметные страницы компонентов, совместимости, конфигуратора и конфигураций пока представлены подготовленными
маршрутами. Реальный пользовательский сценарий уже доступен для создания демо/пустой предметной области, выбора,
редактирования и подтверждённого удаления, а также для настройки типов компонентов и их атрибутов.
