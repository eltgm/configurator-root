...
public class ComponentValidatorImpl implements ComponentValidator {
  ...
  public void validateCreation(
    ...
    List<Component> existedComponents =
        componentType.components() == null ? List.of() : componentType.components();
  ...
  public void validateUpdate(
      ...
      Map<Long, AttributeDefinition> componentTypeAttributesMap) {
    if (!Objects.equals(
        existingComponent.getComponentTypeId(), componentToUpdate.getComponentTypeId())) {
  ...
  private void validateAttributes(
      ...
      ComponentType componentType,
      Map<Long, AttributeDefinition> componentTypeAttributesMap) {
    List<AttributeValue> componentAttributes =
        component.getAttributes() == null ? List.of() : component.getAttributes();
    Map<Long, AttributeValue> componentAttributesMap =
        HashMap.newHashMap(componentAttributes.size());
    ...
    validateIsNoInvalidAttributesForComponent(
        componentAttributesMap, componentTypeAttributesMap, componentType);
  ...
  private void validateIsNameUnique(
      Component component, List<Component> existedComponents, Long ignoredComponentId) {
    boolean isNameNotUnique =
        existedComponents.stream()
            .filter(
                existedComponent ->
                    ignoredComponentId == null
  ...
  private void validateIsNoDuplicate(
      Map<Long, AttributeValue> newComponentAttributesMap,
  ...
  private void validateIsNoInvalidAttributesForComponent(
      Map<Long, AttributeValue> newComponentAttributesMap,
      Map<Long, AttributeDefinition> componentTypeAttributesMap,
      ComponentType componentType) {
    ...
    if (!attributesToAddIdsCopy.isEmpty()) {
      throw new ValidationException(
          "Incorrect attributes for component type {}", componentType.id());
    }
  }

  private void validateHasRequiredAttributes(
      List<AttributeDefinition> attributeDefinitions,
      Map<Long, AttributeValue> newComponentAttributesMap) {
    List<Long> requiredAttributesIds =
    ...
    for (Long requiredAttributesId : requiredAttributesIds) {
      if (!newComponentAttributesMap.containsKey(requiredAttributesId)
          || isEmpty(newComponentAttributesMap.get(requiredAttributesId).value())) {
  ...
  private void validateAttributeValues(
      List<AttributeValue> newComponentAttributes,
      Map<Long, AttributeDefinition> componentTypeAttributesMap) {
    newComponentAttributes.forEach(
        attributeValue -> {
          AttributeDefinition attributeDefinition =
              componentTypeAttributesMap.get(attributeValue.attributeDefinitionId());
