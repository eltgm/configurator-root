...
/**
 * Facade interface for attribute definition management operations. Provides an application boundary
 * for converting transport-layer requests and responses related to {@link AttributeDefinition}
 * entities.
 */
public interface AttributesFacade {
   ...
   * @param createAttributeDefinitionRequest the request containing updated attribute definition
   *     details
   ...
   *     validation fails
   */
  AttributeDefinition updateAttribute(
      Long id, CreateAttributeDefinitionRequest createAttributeDefinitionRequest);

  /**
   * Creates a new attribute definition for the specified component type based on the provided
   * request.
   *
   * @param componentTypeId the unique identifier of the component type for which the attribute
  ...
  List<AttributeDefinition> getAttributesByDomainId(Long domainId);
}
