...
public class ComponentImageValidatorImpl implements ComponentImageValidator {
  ...
  @Override
  public void validateOrder(List<ComponentImage> existingImages, List<Long> orderedImageIds) {
    if (orderedImageIds == null) {
      throw new ValidationException("Image identifiers are required");
    }
    if (orderedImageIds.stream().anyMatch(id -> id == null || id <= 0)) {
      throw new ValidationException("Image identifiers must be positive");
    }
    if (new HashSet<>(orderedImageIds).size() != orderedImageIds.size()) {
      throw new ValidationException("Image identifiers must be unique");
    }
    ...
    Set<Long> existingImageIds =
        existingImages == null
            ...
            : existingImages.stream()
                .map(ComponentImage::id)
                .collect(java.util.stream.Collectors.toSet());
    if (orderedImageIds.size() != existingImageIds.size()
        || !existingImageIds.equals(new HashSet<>(orderedImageIds))) {
      throw new ValidationException("Image identifiers must exactly match the component image set");
    }
  }
