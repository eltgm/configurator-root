...
public class CompatibilityRuleRepositoryImpl implements CompatibilityRuleRepository {
  ...
  public Optional<CompatibilityRuleSet> create(CompatibilityRuleSet ruleSet) {
    Long ruleSetId =
        dslContext
            .insertInto(COMPATIBILITY_RULE_SET)
  ...
  public List<CompatibilityRuleSet> getEnabledByDomainIdAndComponentTypeId(
      Long domainId, Long componentTypeId) {
    return dslContext
        .select(
            COMPATIBILITY_RULE_SET.ID,
            COMPATIBILITY_RULE_SET.DOMAIN_ID,
  ...
  public Optional<CompatibilityRuleSet> updateByIdAndDomainId(
      Long ruleSetId, Long domainId, CompatibilityRuleSet ruleSet) {
    Long updatedRuleSetId;
    try {
      updatedRuleSetId =
          dslContext
              .update(COMPATIBILITY_RULE_SET)
              .set(COMPATIBILITY_RULE_SET.NAME, ruleSet.name())
              .set(COMPATIBILITY_RULE_SET.COMPONENT_TYPE_A_ID, ruleSet.componentTypeAId())
              .set(COMPATIBILITY_RULE_SET.COMPONENT_TYPE_B_ID, ruleSet.componentTypeBId())
              .set(COMPATIBILITY_RULE_SET.ENABLED, ruleSet.enabled())
    ...
    dslContext
        .deleteFrom(COMPATIBILITY_RULE_CONDITION)
  ...
  public boolean deleteByIdAndDomainId(Long ruleSetId, Long domainId) {
    return dslContext
            .deleteFrom(COMPATIBILITY_RULE_SET)
            .where(COMPATIBILITY_RULE_SET.ID.eq(ruleSetId))
            .and(COMPATIBILITY_RULE_SET.DOMAIN_ID.eq(domainId))
            .execute()
        > 0;
  ...
  public boolean existsByBusinessKey(
    ...
    return dslContext.fetchExists(dslContext.selectFrom(COMPATIBILITY_RULE_SET).where(condition));
  }
  ...
  public boolean hasByComponentTypeId(Long componentTypeId) {
    return dslContext.fetchExists(
        dslContext
            .selectFrom(COMPATIBILITY_RULE_SET)
            .where(COMPATIBILITY_RULE_SET.COMPONENT_TYPE_A_ID.eq(componentTypeId))
            .or(COMPATIBILITY_RULE_SET.COMPONENT_TYPE_B_ID.eq(componentTypeId)));
  }

  @Override
  public boolean hasByAttributeDefinitionId(Long attributeDefinitionId) {
    return dslContext.fetchExists(
        dslContext
            .selectFrom(COMPATIBILITY_RULE_CONDITION)
            .where(
                COMPATIBILITY_RULE_CONDITION.LEFT_ATTRIBUTE_DEFINITION_ID.eq(attributeDefinitionId))
            .or(
                COMPATIBILITY_RULE_CONDITION.RIGHT_ATTRIBUTE_DEFINITION_ID.eq(
    ...
    var insertQueries =
        conditions.stream()
            .map(
                condition ->
                    dslContext
                        .insertInto(COMPATIBILITY_RULE_CONDITION)
  ...
  private CompatibilityRuleSet mapRuleSet(Record record) {
    List<CompatibilityRuleCondition> conditions =
        JooqMapperUtils.getListOrNull(record, "conditions");
  ...
  private CompatibilityRuleCondition mapCondition(Record record) {
    return CompatibilityRuleCondition.builder()
        ...
        .operator(
            CompatibilityRuleOperator.valueOf(record.get(COMPATIBILITY_RULE_CONDITION.OPERATOR)))
