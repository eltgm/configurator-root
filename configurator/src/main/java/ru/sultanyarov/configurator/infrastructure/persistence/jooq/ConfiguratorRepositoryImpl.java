...
public class ConfiguratorRepositoryImpl implements ConfiguratorRepository {
  ...
  @Override
  public List<CompatibilityLink> getManualCompatibilityLinks(Long domainId, Long baseComponentId) {
    return dslContext
        .select(
  ...
  private SelectField<List<AttributeValue>> attributeValuesField() {
    return multiset(
            dslContext
                ...
                .join(COMPONENT_TYPE_ATTRIBUTE)
                .on(COMPONENT_TYPE_ATTRIBUTE.ATTRIBUTE_DEFINITION_ID.eq(ATTRIBUTE_DEFINITION.ID))
                .and(COMPONENT_TYPE_ATTRIBUTE.COMPONENT_TYPE_ID.eq(COMPONENT.COMPONENT_TYPE_ID))
                .where(ATTRIBUTE_VALUE.COMPONENT_ID.eq(COMPONENT.ID))
                .orderBy(COMPONENT_TYPE_ATTRIBUTE.ORDER_INDEX, ATTRIBUTE_VALUE.ID))
