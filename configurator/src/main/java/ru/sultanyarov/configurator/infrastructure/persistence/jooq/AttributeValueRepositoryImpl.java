...
public class AttributeValueRepositoryImpl implements AttributeValueRepository {
  private static final ru.sultanyarov.configurator.domain.entity.jooq.tables.AttributeValue AV =
      Tables.ATTRIBUTE_VALUE;

  private final DSLContext dslContext;

  @Override
  public List<AttributeValue> createAttributeValues(
    ...
    for (AttributeValue newComponentAttribute : newComponentAttributes) {
      insert =
          insert.values(
              ...
              newComponentAttribute.dataType() == DataType.STRING
                      || newComponentAttribute.dataType() == DataType.ENUM
                  ? newComponentAttribute.value()
                  : null,
              newComponentAttribute.dataType() == DataType.NUMBER
                  ? NumberUtils.parseNumber(newComponentAttribute.value(), BigInteger.class)
                  : null,
              newComponentAttribute.dataType() == DataType.BOOLEAN
                  ? Boolean.valueOf(newComponentAttribute.value())
                  : null);
    }

    var inserted = insert.returning(AV.ID).fetch(AV.ID);
  ...
  public void deleteByComponentId(Long componentId) {
    dslContext.deleteFrom(AV).where(AV.COMPONENT_ID.eq(componentId)).execute();
  }
  ...
  private RecordMapper<Record, AttributeValue> getAttributeValueRecordMapper() {
    return attributeValueRecord -> {
      ...
      return AttributeValue.builder()
          ...
          .dataType(
              DataType.valueOf(attributeValueRecord.get(Tables.ATTRIBUTE_DEFINITION.DATA_TYPE)))
          .value(value)
          .build();
    };
  }
