...
public class DomainRepositoryImpl implements DomainRepository {
  private static final ru.sultanyarov.configurator.domain.entity.jooq.tables.Domain DOMAIN =
      Tables.DOMAIN;
  ...
  @Override
  public void lockContentsByDomainId(Long id) {
    // FK inserts acquire KEY SHARE locks on their parent. Lock each level before reading
    // the next, so committed concurrent inserts are included and later inserts cannot escape.
    dslContext
        .select(Tables.COMPONENT_TYPE.ID)
        .from(Tables.COMPONENT_TYPE)
        .where(Tables.COMPONENT_TYPE.DOMAIN_ID.eq(id))
        .orderBy(Tables.COMPONENT_TYPE.ID)
        .forUpdate()
        .fetch();
    dslContext
        ...
        .where(
            Tables.COMPONENT.COMPONENT_TYPE_ID.in(
                dslContext
                    .select(Tables.COMPONENT_TYPE.ID)
                    .from(Tables.COMPONENT_TYPE)
                    .where(Tables.COMPONENT_TYPE.DOMAIN_ID.eq(id))))
        .orderBy(Tables.COMPONENT.ID)
        .forUpdate()
        .fetch();
  }

  @Override
  public void deleteContentsByDomainId(Long id) {
    // Remove RESTRICT references before the domain cascades into types/attributes.
    dslContext
        .deleteFrom(Tables.COMPATIBILITY_RULE_SET)
        .where(Tables.COMPATIBILITY_RULE_SET.DOMAIN_ID.eq(id))
        .execute();
    dslContext
  ...
  public void deleteDomainById(Long id) {
    dslContext.delete(DOMAIN).where(DOMAIN.ID.eq(id)).execute();
  }
  ...
  public Page<Domain> getDomains(int page, int pageSize) {
    return jooqPage(
        dslContext,
        dslContext.selectFrom(DOMAIN).orderBy(List.of(DOMAIN.ID.asc())),
        DOMAIN,
        page,
        pageSize,
  ...
  public boolean existsByName(String name) {
    return dslContext.fetchExists(dslContext.selectFrom(DOMAIN).where(DOMAIN.NAME.eq(name)));
  }

  @Override
  public boolean existsById(Long id) {
    return dslContext.fetchExists(dslContext.selectFrom(DOMAIN).where(DOMAIN.ID.eq(id)));
  }
}
